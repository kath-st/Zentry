...
from eventos.models import Reserva, Asiento, Evento, PrecioPorZona, Compra, PurchaseSession
...
class GestorDeReservas:
    ...
    def obtener_precio_reserva(self, reserva: Reserva) -> Decimal:
        """
        Devuelve el precio correspondiente a la combinación evento + zona.
        """
        zona = reserva.asiento.zona

        try:
            ppz = PrecioPorZona.objects.get(evento=reserva.evento, zona=zona)
            return ppz.precio
        except PrecioPorZona.DoesNotExist:
            raise ValueError(
                f"No hay precio configurado para el evento {reserva.evento.id} y zona {zona.id}"
            )
